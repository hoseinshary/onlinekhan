...
import { EXAM_URL as baseUrl } from 'utilities/site-config';

/**
 * find index of object in examGridData by id
 * @param {Number} id
 */
function getIndexById(id) {
  return store.state.examGridData.findIndex(o => o.Id == id);
}
...
const store = {
  ...
  mutations: {
    /**
     * insert new examObj to examGridData
     */
    insert(state, id) {
      let createdObj = util.cloneObject(state.examObj);
      createdObj.Id = id;
      state.examGridData.push(createdObj);
    },

    /**
     * update examObj of examGridData
     */
    update(state) {
      let index = getIndexById(state.selectedId);
      if (index < 0) return;
      util.mapObject(state.examObj, state.examGridData[index]);
    },

    /**
     * delete from examGridData
     */
    delete(state) {
      let index = getIndexById(state.selectedId);
      if (index < 0) return;
      state.examGridData.splice(index, 1);
    },

    /**
     * rest value of examObj
     */
    reset(state, $v) {
      util.clearObject(state.examObj);
      if ($v) {
        $v.$reset();
      }
    }
  ...
  actions: {
    ...
    getByIdStore({ state }, id) {
      axios.get(`${baseUrl}/GetById/${id}`).then(response => {
        state.selectedId = id;
        util.mapObject(response.data, state.examObj);
      });
    },
    ...
    fillGridStore({ state }) {
      ...
      if (state.gridModelChanged) {
        ...
        axios.get(`${baseUrl}/GetAll`).then(response => {
          state.examGridData = response.data;
    ...
    submitCreateStore({ state, commit, dispatch }, closeModal) {
      ...
      dispatch('validateFormStore', vm).then(isValid => {
        ...
        axios.post(`${baseUrl}/Create`, state.examObj).then(response => {
    ...
    submitEditStore({ state, commit, dispatch }) {
      ...
      dispatch('validateFormStore', vm).then(isValid => {
        if (!isValid) return;
        state.examObj.Id = state.selectedId;
        axios.post(`${baseUrl}/Update`, state.examObj).then(response => {
          let data = response.data;
          if (data.MessageType == 1) {
            commit('update');
            dispatch('modelChangedStore');
